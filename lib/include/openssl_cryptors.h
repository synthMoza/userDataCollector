...
public:
    ...
    {
        if (key.GetKeyForEncryption().size() < AES_256_KEY_SIZE + AES_BLOCKSIZE)
            throw std::length_error("Key is too small");

        auto encryptionKey = key.GetKeyForEncryption();
        const unsigned char* openssl_key = &(encryptionKey[0]);
        const unsigned char* openssl_iv = &(encryptionKey[AES_256_KEY_SIZE]);
    ...
    virtual blob_t Decrypt(const blob_t& inputBlob, const IPrivateKey<blob_t>& key) override
    {
        ...
        auto encryptionKey = key.GetKeyForDecryption();
        const unsigned char* openssl_key = &(encryptionKey[0]);
        const unsigned char* openssl_iv = &(encryptionKey[AES_256_KEY_SIZE]);
